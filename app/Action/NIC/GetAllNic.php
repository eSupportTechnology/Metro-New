    ...
    public function __invoke(): array
    {
        try {
            $nicData = DB::table('users')
                ...
                ->whereNotNull('nic_details.user_id')
                ->get();

            $formattedData = $this->formatNicData($nicData);

            return CommonResponse::sendSuccessResponseWithData('NIC Details', $formattedData);
        } catch (\Exception $e) {
            Log::error('GetAllNic Error: ' . $e->getMessage());
            return CommonResponse::sendBadRequestResponse('Failed to retrieve NIC details: ' . $e->getMessage());
        }
    }

    private function formatNicData($nicData): array
    {
        $formattedData = [];

        foreach ($nicData as $data) {
